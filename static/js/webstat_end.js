...
function webp_patch() {
    var zqInputs = document.querySelectorAll('input[type="hidden"]');
    var zq = null;
    for (var i = 0; i < zqInputs.length; i++) {
        var input = zqInputs[i];
        if (input.getAttribute('name') === 'siteid') {
            zq = input;
            break;
        }
    }
    var unreplace = ['cms-30', 'cms-37', 'cms-39', 'drccms-2'];
    if (zq && unreplace.indexOf(zq.value) === -1) {
        replaceUrl();
        (function (open) {
            XMLHttpRequest.prototype.open = function (method, url, async, user, pass) {
                this.addEventListener('readystatechange', function () {
                    if (this.readyState === 4) { // 当请求完成时
                        //console.log('XHR Completed:', method, url);
                        replaceUrl();
                    }
                }, false);
...
function replaceUrl() {
    ...
    for (var i = 0; i < images.length; i++) {
        var img = images[i];
        // 根据实际需求修改这部分，以下是一个简单的示例，将 ".jpg" 替换为 ".webp"
        if (img.src.indexOf("/_fx_/") === -1) {
            img.src = img.src.replace('uploadfile/s', 'uploadfile/_fx_/s');
            img.src = img.src.replace('/statics/', '/statics/_fx_/');
            img.src = img.src.replace('../images/', '../_fx_/images/');
        }
    }
    ...
    for (var i = 0; i < allElements.length; i++) {
        ...
        if (style.backgroundImage !== 'none' && style.backgroundImage.indexOf('/_fx_/') === -1) {
            var backgroundImage = style.backgroundImage;
            var newBackgroundImage = style.backgroundImage.replace('/statics/', '/statics/_fx_/');

            // 设置新的背景图路径
            element.setAttribute('style', 'background-image: ' + newBackgroundImage + ';');
        }
    }
    ...
    for (var j = 0; j < cssFiles.length; j++) {
        ...
        cssFile.addEventListener('load', function () {
            loadedFilesCount++;
            // 当所有 CSS 文件都已加载
            if (loadedFilesCount === cssFiles.length) {
                // 获取所有元素
                var allElements = document.querySelectorAll('*');
                for (var k = 0; k < allElements.length; k++) {
                    var element = allElements[k];
                    var style = window.getComputedStyle(element);
                    // 如果该元素有背景图，并且路径为 '../images/'
                    if (style.backgroundImage !== 'none' && style.backgroundImage.indexOf('/_fx_/') === -1) {
                        var backgroundImage = style.backgroundImage;
                        var newBackgroundImage = style.backgroundImage.replace('/statics/', '/statics/_fx_/');
                        // 设置新的背景图路径
